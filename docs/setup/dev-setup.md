# Development manual for Cowork
This guide will show you how to setup a local development environment for Cowork in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Cowork runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Cowork is available on GitHub [github.com/coworkai/cowork](https://github.com/coworkai/cowork).
- You can download the files using a browser and extract or run `git clone https://github.com/coworkai/cowork` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/coworkai/cowork`, so my project folder is `~/Desktop/cowork`.
...
- Cowork comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/cowork`.
...
## Step 4: Run Cowork in the IDE
Great work! Now you should be able to run Cowork from your IDE including real-time debugging.
...
After inserting my API key in settings, my Cowork instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Cowork in Docker
...
1. Pull the docker image `coworkai/cowork` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/cowork`:
...
Cowork runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running Cowork container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete Cowork development environment! You now have:
...
- The ability to develop, debug, and test Cowork features seamlessly

You're now ready to contribute to Cowork, create custom extensions, or modify the framework to suit your needs. Happy coding!
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t cowork-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`

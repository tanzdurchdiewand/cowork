...
```bash
docker run -p 0:80 -v /path/to/your/work_dir:/a0/usr coworkai/cowork
```
...
Cowork settings can be automatically configured using environment variables with the `A0_SET_` prefix in your `.env` file. This enables automated deployments without manual configuration.

# Quick Start
This guide provides a quick introduction to using Cowork. We'll cover the essential installation steps and running your first Skill.
...
### Step 2: Pull the Cowork Image

Using Docker Desktop GUI, search for `coworkai/cowork` and click Pull, or use the terminal:
...
**Using Docker Desktop:** Go to Images tab, click Run next to `coworkai/cowork`, open Optional settings, map a host port to container port `80` (use `0` for automatic assignment), then click Run.

**Using Terminal:**

```bash
docker run -p 0:80 coworkai/cowork
```
...
![Cowork Web UI](res/setup/6-docker-a0-running-new.png)
...
> [!NOTE]
> Cowork supports any LLM provider, including local models via Ollama. For detailed provider configuration and local model setup, see the [Installation Guide](setup/installation.md#choosing-your-llms).

### Step 5: Start Your First Chat

Once configured, you'll see the Cowork dashboard with access to:
...
Click **New Chat** to start creating with Cowork!

![Cowork Dashboard](res/quickstart/ui_newchat1.png)
...
## Example Interaction
Let's ask Cowork to use one of the built-in skills. Here's how:

1. Type "Activate your brainstorming skill" in the chat input field and press Enter or click the send button.
2. Cowork will process your request. You'll see its thoughts and tool calls in the UI.
...
## Next Steps
Now that you've run a simple task, you can experiment with more complex requests. Try asking Cowork to:
